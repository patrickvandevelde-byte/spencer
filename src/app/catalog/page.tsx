  ...
  return (
    <div className="space-y-16">
      ...
      <section>
        ...
        <h1 className="mb-4 text-4xl font-semibold tracking-tight text-[var(--fg-bright)]">
          Actuator &amp; Fluid Library
        </h1>
        <p className="mb-2 max-w-2xl text-[var(--fg-secondary)]">
          Browse the full hardware and fluid database. Click any actuator or fluid to start a configuration.
        </p>
        <p className="max-w-2xl text-sm text-[var(--muted)]">
          Newtonian + Non-Newtonian fluids &middot; Dv10/Dv50/Dv90 droplet distribution &middot; Material compatibility
        </p>
      </section>
      ...
      <section className="grid grid-cols-2 gap-4 sm:grid-cols-5">
        {[
          ...
          <div key={s.label} className="rounded-2xl bg-[var(--bg-secondary)] p-5 text-center">
      ...
      <section>
        <div className="relative">
          ...
          <input
            ...
            className="input-field w-full rounded-xl py-3.5 pl-12 pr-6 text-sm"
          />
        </div>
      </section>
      ...
      <section className="space-y-6">
        <div className="flex items-end justify-between">
          <div>
            <h2 className="mb-2 text-2xl font-semibold text-[var(--fg-bright)]">Actuator Catalog</h2>
            ...
            {ALL_INDUSTRIES.map((ind) => {
              ...
              return (
                <button key={ind} onClick={() => setIndustryFilter(ind)}
                  className={`rounded-full border px-3.5 py-1.5 text-xs font-medium transition-all ${
                    industryFilter === ind ? "border-[var(--accent)] bg-[var(--accent)]/8 text-[var(--accent)]" : "border-[var(--border)] text-[var(--muted)] hover:border-[var(--border-hover)]"
                  }`}
                >
                  {INDUSTRY_LABELS[ind]}
                </button>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredActuators.map((a) => {
            const color = ACTUATOR_COLORS[a.type] || "var(--accent)";
            return (
              <Link
                key={a.id}
              ...
              >
                ...
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold" style={{ color }}>{a.sku}</span>
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${a.manufacturer === "Coster" ? "bg-[#ec4899]/8 text-[#ec4899]" : "bg-[var(--accent)]/8 text-[var(--accent)]"}`}>
                    {a.manufacturer}
                  </span>
                </div>
                ...
                <div className="mt-3 space-y-1.5 text-xs">
                  <div className="flex justify-between"><span className="text-[var(--muted)]">Orifice</span><span className="font-medium text-[var(--fg)]">{a.orificeDiameter_mm} mm</span></div>
                  <div className="flex justify-between"><span className="text-[var(--muted)]">Max Pressure</span><span className="font-medium text-[var(--fg)]">{a.maxPressure_bar} bar</span></div>
                  <div className="flex justify-between"><span className="text-[var(--muted)]">Price</span><span className="font-medium text-[var(--fg)]">${a.price_usd.toFixed(2)}</span></div>
                </div>

                <div className="mt-3 flex flex-wrap gap-1">
                  {a.materialCompatibility.slice(0, 3).map((mat) => (
                    <span key={mat} className="rounded-full bg-[var(--bg-secondary)] px-2 py-0.5 text-[10px] font-medium text-[var(--muted)]">{mat}</span>
                  ))}
                  {a.materialCompatibility.length > 3 && (
                    <span className="text-[var(--muted)] text-[10px]">+{a.materialCompatibility.length - 3}</span>
                  )}
                </div>
              </Link>
            );
      ...
      <section className="space-y-6">
        <div>
          <h2 className="mb-2 text-2xl font-semibold text-[var(--fg-bright)]">Fluid Library</h2>
          <p className="text-sm text-[var(--muted)]">
            {FLUIDS.length} fluids ({nonNewtonianCount} non-Newtonian) &mdash; click a row to configure
          </p>
        </div>

        <div className="flex flex-wrap gap-2">
          <button onClick={() => setSolventFilter("all")}
            className={`rounded-full border px-3.5 py-1.5 text-xs font-medium transition-all ${solventFilter === "all" ? "border-[var(--accent)] bg-[var(--accent)]/8 text-[var(--accent)]" : "border-[var(--border)] text-[var(--muted)] hover:border-[var(--border-hover)]"}`}>
            All ({FLUIDS.length})
          </button>
          {ALL_SOLVENT_CLASSES.map((sc) => {
            ...
            return (
              <button key={sc} onClick={() => setSolventFilter(sc)}
                className={`rounded-full border px-3.5 py-1.5 text-xs font-medium transition-all ${solventFilter === sc ? "border-[var(--accent)] bg-[var(--accent)]/8 text-[var(--accent)]" : "border-[var(--border)] text-[var(--muted)] hover:border-[var(--border-hover)]"}`}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[var(--bg-secondary)]">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)]">Viscosity</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)]">Density</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)]">Tension</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)]">Rheology</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)]">Class</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)]">Hazards</th>
                </tr>
              </thead>
              <tbody>
                {filteredFluids.map((f) => (
                  <Link key={f.id} href={`/configure?fluid=${f.id}`} className="table-row-hover border-b border-[var(--border)] last:border-b-0 no-underline cursor-pointer" style={{ display: "table-row" }}>
                    <td className="px-4 py-3 text-[var(--fg-bright)] font-medium">{f.name}</td>
                    <td className="px-4 py-3 font-[family-name:var(--font-mono)] text-xs">{f.viscosity_cP} cP</td>
                    <td className="px-4 py-3 font-[family-name:var(--font-mono)] text-xs">{f.density_kg_m3}</td>
                    <td className="px-4 py-3 font-[family-name:var(--font-mono)] text-xs">{f.surfaceTension_mN_m}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium ${f.rheology !== "newtonian" ? "bg-[var(--accent)]/8 text-[var(--accent)]" : "bg-[var(--bg-secondary)] text-[var(--muted)]"}`}>
                      ...
                      {f.hazards.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {f.hazards.slice(0, 2).map((h) => (
                            <span key={h} className="rounded-full bg-[var(--danger)]/8 px-2 py-0.5 text-[10px] font-medium text-[var(--danger)]">{h}</span>
                          ))}
                          {f.hazards.length > 2 && <span className="text-[var(--danger)] text-[10px]">+{f.hazards.length - 2}</span>}
                        </div>
      ...
      {/* CTA */}
      <section className="rounded-2xl bg-[var(--bg-secondary)] p-10 text-center">
        <h2 className="mb-3 text-2xl font-semibold text-[var(--fg-bright)]">
          Ready to configure?
        </h2>
        <p className="mb-6 text-[var(--muted)]">
          Select from the library or enter custom fluid properties.
        </p>
        <Link
          href="/configure"
          className="btn-primary inline-flex items-center gap-2 px-7 py-3 text-sm font-medium no-underline"
        >
          Open Configurator
